...
public class MajorGroupCreatePanel extends JPanel {
    private static final Pattern COMBO_CODE_PATTERN = Pattern.compile("^[A-Z]\\d{2}$");
    private static final Pattern COMBO_NAME_PATTERN = Pattern.compile("^[\\p{L}]+(?:[\\p{L}\\s]*[\\p{L}])?(?:\\s*\\([\\p{L}\\s]+\\))?$");
    private static final String[] FALLBACK_SUBJECT_CODES = {
            "TO", "LI", "HO", "SI", "SU", "DI", "VA", "TI",
            "KTPL", "CNCN", "CNNN", "N1", "NK1", "NK2"
    };

    private final SubjectGroupBUS subjectGroupBUS = new SubjectGroupBUS();
    private JTextField txtMajorCode;
    ...
    public MajorGroupCreatePanel(Consumer<MajorGroupDTO> onSave, Runnable onCancel) {
        ...
        JLabel titleLabel = new JLabel("Thêm tổ hợp");
        ...
        JPanel majorRow = new JPanel(new GridLayout(1, 1, 0, 0));
        majorRow.setBackground(Color.WHITE);
        txtMajorCode = createStyledTextField();
        majorRow.add(createFieldGroup("Mã ngành", txtMajorCode));
        ...
        String[] subjectOptions = buildSubjectOptions();
        cmbSubj1 = createStyledComboBox(subjectOptions);
        cmbSubj2 = createStyledComboBox(subjectOptions);
        cmbSubj3 = createStyledComboBox(subjectOptions);
        ...
        btnSave.addActionListener(e -> handleSave(onSave));

        footerPanel.add(btnCancel);
        footerPanel.add(btnSave);

        add(titlePanel, BorderLayout.NORTH);
        add(topAlignedContainer, BorderLayout.CENTER);
    ...
    private String[] buildSubjectOptions() {
    ...
    private void handleSave(Consumer<MajorGroupDTO> onSave) {
        ...
        if (maNganh.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Vui lòng nhập mã ngành", "Lỗi", JOptionPane.ERROR_MESSAGE);
            return;
        }
        if (maToHop.isEmpty()) {
        ...
        if (!COMBO_NAME_PATTERN.matcher(tenToHop).matches()) {
            JOptionPane.showMessageDialog(
                    this,
                    "Tên ngành ko đc là số",
                    "Lỗi",
